...
| 术语 | 统一定义 |
|------|----------|
| 核心模块 | 42 个可独立学习与演示的数据结构/算法模块（`L/T/H/G/S/SR/ST/P` 编码体系） |
| 附属页面 | 非核心模块的专题页面，当前固定为 2 个：`/modules/sorting`、`/about` |
| 系统页面 | 框架级页面与路由入口，包含 `/`、`/modules`、`*`（404） |
| 模块导航页 | `/modules`，用于分类浏览、搜索、筛选模块 |
| 模块详情页 | `/modules/{slug}` 对应的具体模块可视化页面 |
| 排序总览页 | `/modules/sorting`，展示排序算法对比与复杂度总览 |
...
| 版本 | 日期 | 主要变化 |
|-----|------|---------|
| v1.1 | 2026-03-03 | 新增跨文档术语基线，统一页面与模块定义口径 |
