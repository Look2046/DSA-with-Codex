...
import { buildRabinKarpTimelineFromPreset } from './rabinKarpTimelineAdapter';
...
function runReplaySequence(speedMs: number): {
  ...
  const frames = buildRabinKarpTimelineFromPreset('classic');
...
describe('rabinKarp timeline deterministic replay', () => {
  it('keeps the same frame order across speed changes after seek/resume', () => {
    ...
    expect(slow.finalMatches).toEqual([0, 9, 12]);
    expect(fast.finalMatches).toEqual([0, 9, 12]);
  });
});
