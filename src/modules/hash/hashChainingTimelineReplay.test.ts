...
import { buildHashChainingTimelineFromOperation } from './hashChainingTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildHashChainingTimelineFromOperation('insert');
...
describe('hash chaining timeline deterministic replay', () => {
