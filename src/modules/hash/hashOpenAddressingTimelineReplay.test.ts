...
import { buildHashOpenAddressingTimelineFromOperation } from './hashOpenAddressingTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildHashOpenAddressingTimelineFromOperation('insert');
...
describe('hash open addressing timeline deterministic replay', () => {
  it('keeps the insert frame order stable across speed changes with seek/resume', () => {
