...
import { buildBinarySearchTimelineFromInput } from './binarySearchTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string } {
  const frames = buildBinarySearchTimelineFromInput([1, 3, 5, 7, 9, 11], 7);
...
describe('binary-search timeline deterministic replay', () => {
