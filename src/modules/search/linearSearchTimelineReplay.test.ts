...
import { buildLinearSearchTimelineFromInput } from './linearSearchTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string } {
  const frames = buildLinearSearchTimelineFromInput([9, 4, 7, 1, 8, 3], 8);
...
describe('linear-search timeline deterministic replay', () => {
