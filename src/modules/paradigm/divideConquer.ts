...
export function generateDivideConquerSteps(presetId: DivideConquerPresetId): DivideConquerStep[] {
  ...
  function solve(start: number, end: number, parentStack: Array<[number, number]>): DivideConquerResolution {
    ...
    if (start === end) {
      ...
      resolved.push(base);
    steps.push(createStep(preset, 'baseCase', [2], [start, end], null, null, currentStack, resolved, base));
      return base;
    }
    ...
    steps.push(createStep(preset, 'split', [3], [start, end], leftRange, rightRange, currentStack, resolved, null));
