...
import { buildSelectionSortTimelineFromInput } from './selectionTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string } {
  const frames = buildSelectionSortTimelineFromInput([9, 2, 7, 1]);
...
describe('selection timeline deterministic replay', () => {
