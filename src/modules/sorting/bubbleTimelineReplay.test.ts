...
import { buildBubbleSortTimelineFromInput } from './bubbleTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string } {
  const frames = buildBubbleSortTimelineFromInput([9, 2, 7, 1]);
...
describe('bubble timeline deterministic replay', () => {
