...
describe('avl timeline deterministic replay', () => {
  it('keeps frame order stable across speed changes when using seek/resume', () => {
    ...
    expect(slow.finalOutcome).toBe('rebalanced');
    expect(fast.finalOutcome).toBe('rebalanced');
  });
});
