...
import { buildBTreeComparisonTimelineFromInput } from './btreeComparisonTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildBTreeComparisonTimelineFromInput([10, 20, 5, 6, 12, 30], 7);
...
describe('b-tree comparison deterministic replay', () => {
  it('keeps frame order stable across speed changes when using seek/resume', () => {
    ...
    expect(slow.finalOutcome).toBe('inserted');
    expect(fast.finalOutcome).toBe('inserted');
  });
});
