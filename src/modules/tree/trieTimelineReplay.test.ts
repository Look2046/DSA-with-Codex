...
import { buildTrieTimelineFromInput } from './trieTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildTrieTimelineFromInput(['to', 'tea', 'ted', 'ten'], 'team', 'team');
...
describe('trie timeline deterministic replay', () => {
  it('keeps frame order stable across speed changes when using seek/resume', () => {
    ...
    expect(slow.finalOutcome).toBe('found');
    expect(fast.finalOutcome).toBe('found');
  });
});
