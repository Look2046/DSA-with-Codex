...
import { buildHeapTimelineFromInput } from './heapTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildHeapTimelineFromInput([40, 25, 35, 10, 15], 'insert', 50);
...
describe('heap timeline deterministic replay', () => {
