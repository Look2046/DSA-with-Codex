...
import { buildStackTimelineFromInput } from './stackTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string } {
  const frames = buildStackTimelineFromInput([3, 8, 1], { type: 'push', value: 9 });
...
describe('stack timeline deterministic replay', () => {
