...
export type QueueMode = 'normal' | 'linked' | 'circular';
...
export type QueueStep = AnimationStep & {
  ...
  rearIndex: number;
  size: number;
  action: 'initial' | 'prepareEnqueue' | 'linkEnqueue' | 'enqueue' | 'dequeue' | 'front' | 'completed';
  indices: number[];
...
export function generateQueueSteps(
  ...
  if (operation.type === 'enqueue') {
    ...
    if (mode === 'linked') {
      const oldRearIndex = runtime.size > 0 ? runtime.size - 1 : -1;
      steps.push(
        snapshot(runtime, 'prepareEnqueue', [1], [], {
      ...
      steps.push(
        snapshot(
          ...
          'linkEnqueue',
          runtime.size === 0 ? [2] : [3],
          oldRearIndex >= 0 ? [{ index: oldRearIndex, type: 'moving' as const }] : [],
          {
            enqueuedValue: operation.value,
            frontValue: runtime.queueState[0],
          },
    ...
    steps.push(
      snapshot(runtime, 'enqueue', mode === 'linked' ? [4] : [3], [{ index: insertedIndex, type: 'new-node' }], {
        enqueuedValue: operation.value,
        frontValue: runtime.queueState[0],
      }),
    );
