...
describe('generateQueueSteps', () => {
  ...
  it('animates linked enqueue as creating a side node before linking it after rear', () => {
    const steps = generateQueueSteps([3, 8, 1], { type: 'enqueue', value: 9 }, 'linked');

    expect(steps.map((step) => step.action)).toEqual(['initial', 'prepareEnqueue', 'linkEnqueue', 'enqueue', 'completed']);
    ...
    expect(steps[2]).toMatchObject({
      queueState: [3, 8, 1],
      codeLines: [3],
      highlights: [{ index: 2, type: 'moving' }],
      enqueuedValue: 9,
    });
    expect(steps[3]).toMatchObject({
      queueState: [3, 8, 1, 9],
      codeLines: [4],
      highlights: [{ index: 3, type: 'new-node' }],
    });
  });

  it('animates empty linked enqueue with front and rear both pointing to the new node', () => {
    const steps = generateQueueSteps([], { type: 'enqueue', value: 9 }, 'linked');
    ...
    expect(steps[3]).toMatchObject({
      queueState: [9],
      codeLines: [4],
      highlights: [{ index: 0, type: 'new-node' }],
      frontIndex: 0,
      rearIndex: 1,
    });
  });
