...
import { buildDijkstraTimelineFromPreset } from './dijkstraTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildDijkstraTimelineFromPreset('positiveDirected');
...
describe('dijkstra timeline deterministic replay', () => {
