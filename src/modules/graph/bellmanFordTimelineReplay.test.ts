...
import { buildBellmanFordTimelineFromPreset } from './bellmanFordTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildBellmanFordTimelineFromPreset('negativeDirected');
...
describe('bellmanFord timeline deterministic replay', () => {
