...
import { buildGraphRepresentationTimelineFromPreset } from './graphRepresentationTimelineAdapter';

function runReplaySequence(speedMs: number): { sequence: number[]; finalStatus: string; finalOutcome: string | undefined } {
  const frames = buildGraphRepresentationTimelineFromPreset('undirected');
...
describe('graph representation timeline deterministic replay', () => {
