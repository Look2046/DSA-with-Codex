...
const modules: ModuleMetadata[] = [
  ...
  { id: 'H-01', name: 'Hash Table - Chaining', route: '/modules/hash-chaining', category: 'search', difficulty: 1, implemented: true },
...
describe('modulesPageUtils', () => {
  it('filters modules by category', () => {
    ...
    expect(filterModules(modules, 'search').map((item) => item.id)).toEqual(['SR-02', 'H-01']);
    ...
    expect(filterModules(modules, 'hash').map((item) => item.id)).toEqual([]);
