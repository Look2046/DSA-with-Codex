...
type StackPageMode = 'sequential' | 'linked';

const STACK_TITLE_KEYS: Record<StackPageMode, TranslationKey> = {
  sequential: 'module.l04a.title',
  linked: 'module.l04b.title',
};

const STACK_BODY_KEYS: Record<StackPageMode, TranslationKey> = {
  sequential: 'module.l04a.body',
  linked: 'module.l04b.body',
};

const SEQUENTIAL_STACK_PSEUDOCODE = {
  push: [
    { sourceLine: 1, textKey: 'module.l04.sequentialCode.line1', cLine: 'if (top < 0 || top > MAX) return ERROR;' },
    { sourceLine: 2, textKey: 'module.l04.sequentialCode.line2', cLine: '/* push(value) */' },
  ...
  pop: [
    { sourceLine: 1, textKey: 'module.l04.sequentialCode.line1', cLine: 'if (top < 0 || top > MAX) return ERROR;' },
    { sourceLine: 5, textKey: 'module.l04.sequentialCode.line5', cLine: 'if (top == 0) return UNDERFLOW; --top; return data[top];' },
  ],
  peek: [
    { sourceLine: 1, textKey: 'module.l04.sequentialCode.line1', cLine: 'if (top < 0 || top > MAX) return ERROR;' },
    { sourceLine: 6, textKey: 'module.l04.sequentialCode.line6', cLine: 'if (top == 0) return EMPTY; return data[top - 1];' },
  ],
} as const;

const LINKED_STACK_PSEUDOCODE = {
  push: [
    { sourceLine: 1, textKey: 'module.l04.linkedCode.line1', cLine: '/* push(value) */' },
    { sourceLine: 2, textKey: 'module.l04.linkedCode.line2', cLine: 'Node *s = createNode(value);' },
    { sourceLine: 3, textKey: 'module.l04.linkedCode.line3', cLine: 's->next = top;' },
    { sourceLine: 4, textKey: 'module.l04.linkedCode.line4', cLine: 'top = s;' },
  ],
  pop: [
    { sourceLine: 5, textKey: 'module.l04.linkedCode.line5', cLine: '/* pop() */' },
    { sourceLine: 6, textKey: 'module.l04.linkedCode.line6', cLine: 'if (top == NULL) return EMPTY;' },
    { sourceLine: 7, textKey: 'module.l04.linkedCode.line7', cLine: 'Node *p = top; top = top->next; free(p);' },
  ],
  peek: [
    { sourceLine: 8, textKey: 'module.l04.linkedCode.line8', cLine: 'if (top == NULL) return EMPTY; return top->data;' },
  ],
} as const;
...
export function StackPage() {
  return <StackWorkbenchPage mode="sequential" />;
}

export function SequentialStackPage() {
  return <StackWorkbenchPage mode="sequential" />;
}

export function LinkedStackPage() {
  return <StackWorkbenchPage mode="linked" />;
}
...
const LINKED_STACK_PREVIEW_GAP = 10;
...
export function getLinkedStackPreviewConnectorPoints(
  ...
  targetRect: Pick<DOMRect, 'top' | 'right' | 'height'>,
  zoom = 1,
  ...
  return {
    start: {
      x: (floatingRect.left - sceneRect.left - LINKED_STACK_PREVIEW_GAP) / scale,
      y: (floatingRect.top + floatingRect.height / 2 - sceneRect.top) / scale,
    },
    end: {
      x: (targetRect.right - sceneRect.left + LINKED_STACK_PREVIEW_GAP) / scale,
      y: (targetRect.top + targetRect.height / 2 - sceneRect.top) / scale,
    },
  };
}
...
function StackWorkbenchPage({ mode }: { mode: StackPageMode }) {
  const { t } = useI18n();
  ...
  const linkedActiveDisplayLines = linkedPseudocode
    ...
    .filter((line) => line > 0);
  const activeTitle = t(STACK_TITLE_KEYS[mode]);
  const activeDescription = t(STACK_BODY_KEYS[mode]);
  const activeLaneLabel = mode === 'sequential' ? t('module.l04.compare.sequential') : t('module.l04.compare.linked');
  const activeSizeLabel = mode === 'sequential' ? `${currentSequentialSize}/${STACK_CAPACITY}` : String(currentLinkedSize);
  const activeOutcomeLabel = mode === 'sequential' ? sequentialOutcomeLabel : linkedOutcomeLabel;
  const activeStackText = mode === 'sequential' ? formatStack(currentSequentialStack) : formatStack(currentLinkedStack);
  const activeHighlightSummary = mode === 'sequential' ? sequentialHighlightSummary : linkedHighlightSummary;
  ...
  useLayoutEffect(() => {
    ...
    const updatePath = () => {
      ...
      const zoomValue = Number.parseFloat(
        getComputedStyle(scene).getPropertyValue('--workspace-stage-zoom') || '1',
      );
      const { start, end } = getLinkedStackPreviewConnectorPoints(sceneRect, floatingRect, targetRect, zoomValue);
    ...
    <WorkspaceShell
      ...
      stageMeta={
        <>
          ...
          <span className="tree-workspace-pill">
            {activeLaneLabel}: {activeSizeLabel}
          </span>
      ...
      controlsContent={
        <>
          <div className="array-controls-grid stack-controls-grid">
            ...
            <label className="tree-workspace-field array-controls-field stack-controls-field-speed" htmlFor="speed-select-StackPage">
              <span>{t('module.s01.speed')}</span>
              <select
                id="speed-select-StackPage"
                value={speedMs}
          ...
          <div className="linear-controls-section">
            <div className="linear-controls-summary">
              ...
              <div className="linear-controls-card">
                ...
                <strong>{activeOutcomeLabel}</strong>
              </div>
              <div className="linear-controls-card">
            ...
            <div className="linear-controls-note-grid">
              <p className="linear-controls-note linear-controls-note-wide">
                {stepDescription} · {stepNote}
              </p>
              <p className="linear-controls-note">
                {activeLaneLabel}: {activeStackText} | {t('module.s01.highlight')}: {activeHighlightSummary}
              </p>
            </div>
      ...
      stageContent={
        <div className={`stack-compare-stage stack-compare-stage-single stack-compare-stage-${mode}`}>
          {mode === 'sequential' ? (
            <section className="stack-compare-lane">
            <div className="stack-compare-lane-body">
              <div className="stack-stage-inline">
                ...
                <div className="stack-stage-caption" aria-label="sequential-stack-summary">
                  <p>
                    <span>{t('module.l04.stage.currentState')}</span>
                    <strong>{getOutcomeLabel(currentSnapshot?.sequential.outcome ?? 'ok', t)}</strong>
                  </p>
                  <p>
                    <span>{t('module.l04.stage.currentSize')}</span>
                    <strong>{currentSequentialSize}/{STACK_CAPACITY}</strong>
                  </p>
                </div>
              </div>
            </div>
          ...
          {mode === 'linked' ? (
            ...
            <div className="stack-compare-lane-body">
              <div className="stack-stage-inline">
                ...
                <div className="stack-stage-caption" aria-label="linked-stack-summary">
                  <p>
                    <span>{t('module.l04.stage.currentState')}</span>
                    <strong>{getOutcomeLabel(currentSnapshot?.linked.outcome ?? 'ok', t)}</strong>
                  </p>
                  <p>
                    <span>{t('module.l04.stage.currentSize')}</span>
