...
function createRandomWord(existingWords: string[]): string {
  const taken = new Set(existingWords.map((word) => word.toLowerCase()));
  const prefixes = ['a', 'be', 'ca', 'do', 'fa', 'go', 'ki', 'la', 'mi', 'no', 'pa', 're', 'so', 'ta'];
  const middles = ['l', 'm', 'n', 'p', 'r', 's', 't', 'v', 'z', 'ck', 'nd', 'nt', 'pl', 'st'];
  const suffixes = ['an', 'at', 'ed', 'en', 'er', 'et', 'in', 'it', 'on', 'or', 'um', 'us', 'y'];
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    const suffix = suffixes[Math.floor(Math.random() * suffixes.length)] ?? 't';
    const word = `${prefix}${middle}${suffix}`.slice(0, 8);
    if (!taken.has(word) && isValidWord(word)) {
      return word;
    }
  }

  for (let index = 0; index < 999; index += 1) {
    const word = `w${index}`.replace(/\d/g, (digit) => String.fromCharCode(97 + Number(digit)));
    if (!taken.has(word) && isValidWord(word)) {
      return word;
    }
  }
...
function appendSortedUniqueWords(words: string[], word: string): string[] {
  return Array.from(new Set([...words, word])).sort((left, right) => left.localeCompare(right));
}
...
export function TriePage() {
  const { t } = useI18n();
  const autoNextInsertWordKeyRef = useRef<string | null>(null);
  ...
  const currentInsertWasAdded = useMemo(
    () => steps.some((step) => step.phase === 'insert' && step.outcome === 'inserted'),
    [steps],
  );
  ...
  useEffect(() => {
    if (
      status !== 'completed' ||
      !isAtLastFrame ||
      !currentInsertWasAdded ||
      activeConfig.seedWords.includes(activeConfig.insertWord)
    ) {
      return;
    }

    const nextKey = `${activeConfig.insertWord}:${activeConfig.seedWords.join(',')}`;
    if (autoNextInsertWordKeyRef.current === nextKey) {
      return;
    }

    autoNextInsertWordKeyRef.current = nextKey;
    const timerId = window.setTimeout(() => {
      const nextSeedWords = appendSortedUniqueWords(activeConfig.seedWords, activeConfig.insertWord);
      const nextWord = createRandomWord(nextSeedWords);
      reset();
      setActiveConfig({
        seedWords: nextSeedWords,
        insertWord: nextWord,
        queryWord: nextWord,
      });
      setInsertInput(nextWord);
      setQueryInput(nextWord);
      setError('');
    }, 0);

    return () => window.clearTimeout(timerId);
  }, [
    activeConfig.insertWord,
    activeConfig.seedWords,
    currentInsertWasAdded,
    isAtLastFrame,
    reset,
    status,
  ...
  const handleApply = () => {
    ...
    setActiveConfig({
      seedWords: activeConfig.seedWords,
      insertWord,
      queryWord,
    });
    setError('');
    ...
    <WorkspaceShell
      ...
      controlsContent={
        <div className="tree-controls-workbench">
          ...
          <label className="tree-workspace-field" htmlFor="speed-select-TriePage">
            <span>{t('module.s01.speed')}</span>
            <select
              id="speed-select-TriePage"
              value={speedMs}
