...
export function resolveLinkedListConfig(
  ...
  if (operationType === 'find') {
    ...
    return { config: { mode, list: parsedList, operation: { type: 'find', value } }, error: '' };
  }
  ...
  if (operationType === 'insertAt') {
    ...
    return { config: { mode, list: parsedList, operation: { type: 'insertAt', index: displayIndex - 1, value } }, error: '' };
  }

  const displayIndex = Number(indexInput);
  if (parsedList.length === 0) {
    if (displayIndex === 1) {
      return { config: { mode, list: [], operation: { type: 'deleteAt', index: 0 } }, error: '' };
    }
  ...
  return { config: { mode, list: parsedList, operation: { type: 'deleteAt', index: displayIndex - 1 } }, error: '' };
}
...
export function resolveLinkedListConfigFromJson(rawJson: string, t: Translator): JsonParseResult<LinkedListConfig> {
  ...
  const mode = typeof parsed.mode === 'string' ? parsed.mode : 'singly';
  if (mode !== 'singly' && mode !== 'doubly' && mode !== 'circular') {
    return { config: null, error: t('module.l03.json.error.schema') };
  }
  ...
  if (operationType === 'find') {
    ...
    return resolveLinkedListConfig(mode, listInput, operationType, String(parsed.operation.value), '', t);
  }
  ...
  if (operationType === 'deleteAt') {
    ...
    return resolveLinkedListConfig(mode, listInput, operationType, '', String(parsed.operation.index + 1), t);
  }
