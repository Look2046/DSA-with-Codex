...
import { buildDfsTimelineFromPreset } from '../../modules/graph/dfsTimelineAdapter';
import { getGraphEdgeKey, getGraphPresetIds, type GraphDefinition, type GraphPresetId } from '../../modules/graph/graphRepresentation';
import type { DfsStep } from '../../modules/graph/dfs';
import type { PlaybackStatus } from '../../types/animation';
import { getGraphStageEdgeGeometry } from './graphStageGeometry';

const DEFAULT_PRESET: GraphPresetId = 'dag';
const CODE_LINE_KEYS = [
  'module.g02.code.line1',
  'module.g02.code.line2',
  'module.g02.code.line3',
  'module.g02.code.line4',
  'module.g02.code.line5',
  'module.g02.code.line6',
  'module.g02.code.line7',
  'module.g02.code.line8',
] as const;
const SPEED_OPTIONS = [
...
function getStepDescription(step: DfsStep | undefined, t: TranslateFn): string {
  if (!step) {
    return '-';
  }
  ...
  switch (step.action) {
    case 'initial':
      return t('module.g02.step.initial');
    case 'pushStart':
      return t('module.g02.step.pushStart');
    case 'visit':
      return t('module.g02.step.visit');
    case 'inspectNeighbor':
      return t('module.g02.step.inspectNeighbor');
    case 'descend':
      return t('module.g02.step.descend');
    case 'skipVisited':
      return t('module.g02.step.skipVisited');
    case 'backtrack':
      return t('module.g02.step.backtrack');
    case 'completed':
      return t('module.g02.step.completed');
    default:
      return '-';
  }
}
...
export function DfsPage() {
  ...
  const timelineFrames = useMemo(() => buildDfsTimelineFromPreset(presetId), [presetId]);
  ...
  const stackSet = useMemo(() => new Set(currentSnapshot?.stackNodeIndices ?? []), [currentSnapshot?.stackNodeIndices]);
  const visitOrderIndexMap = useMemo(
    () =>
      new Map(
  ...
  const currentRowText =
    graph && currentSnapshot?.activeNodeIndex !== null && currentSnapshot?.activeNodeIndex !== undefined
      ...
      : '-';
  const stackText =
    graph && (currentSnapshot?.stackNodeIndices.length ?? 0) > 0
      ? currentSnapshot?.stackNodeIndices.map((index) => graph.nodes[index]?.id ?? '?').join(' -> ') ?? '-'
      : '-';
  const outputText =
  ...
  const stackDepth = currentSnapshot?.stackNodeIndices.length ?? 0;
  const totalNodes = graph?.nodes.length ?? 0;
  const isAtLastFrame = steps.length === 0 || currentStep >= steps.length - 1;

  const detailParts = [
    `${t('module.g02.meta.topology')}: ${graph ? getTopologyLabel(graph, t) : '-'}`,
    `${t('module.g02.meta.visitedCount')}: ${visitedCount}/${totalNodes}`,
    `${t('module.g02.meta.stackDepth')}: ${stackDepth}`,
  ];

  if (activeRelationLabel !== '-') {
    detailParts.push(`${t('module.g02.meta.activeRelation')}: ${activeRelationLabel}`);
  }
  if (stackText !== '-') {
    detailParts.push(`${t('module.g02.meta.stack')}: ${stackText}`);
  }

  return (
    <WorkspaceShell
      pageClassName="array-page tree-page bst-page"
      title={t('module.g02.title')}
      description={t('module.g02.body')}
      stageAriaLabel={t('module.g02.stage')}
      ...
      stageMeta={
        <>
          ...
          <span className="tree-workspace-pill">
            {t('module.g02.meta.preset')}: {graph ? getPresetLabel(graph.presetId, t) : '-'}
          </span>
          <span className="tree-workspace-pill">
            {t('module.g02.meta.current')}: {activeNodeLabel}
          </span>
          <span className="tree-workspace-pill">
            {t('module.g02.meta.stackDepth')}: {stackDepth}
          </span>
      ...
      controlsContent={
        <>
          ...
          <label className="tree-workspace-field" htmlFor="speed-select-DfsPage">
            <span>{t('module.s01.speed')}</span>
            <select
              id="speed-select-DfsPage"
              value={speedMs}
          ...
          {graph ? (
            <>
              ...
              <div className="tree-workspace-sample-block">
                ...
                <code>
                  {t('module.g02.meta.start')}: {startNodeLabel} · {t('module.g01.meta.vertexCount')}: {graph.nodes.length}{' '}
      ...
      stepContent={
        <>
          ...
          <dl className="tree-workspace-kv">
            ...
            <div>
              <dt>{t('module.g02.meta.preset')}</dt>
              <dd>{graph ? getPresetLabel(graph.presetId, t) : '-'}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.topology')}</dt>
              <dd>{graph ? getTopologyLabel(graph, t) : '-'}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.start')}</dt>
              <dd>{startNodeLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.current')}</dt>
              <dd>{activeNodeLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.neighbor')}</dt>
              <dd>{activeNeighborLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.activeRelation')}</dt>
              <dd>{activeRelationLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.currentRow')}</dt>
              <dd>{currentRowText}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.stack')}</dt>
              <dd>{stackText}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.output')}</dt>
              <dd>{outputText}</dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.visitedCount')}</dt>
              <dd>
                {visitedCount}/{totalNodes}
              </dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.completedCount')}</dt>
              <dd>
                {completedCount}/{totalNodes}
              </dd>
            </div>
            <div>
              <dt>{t('module.g02.meta.stackDepth')}</dt>
              <dd>{stackDepth}</dd>
            </div>
          </dl>

          <div className="tree-workspace-code-block">
            <span className="tree-workspace-code-title">{t('module.g02.code.title')}</span>
      ...
      stageContent={
        graph ? (
          <div className="graph-dfs-stage-scene" aria-hidden="true">
            <section className="graph-stage-canvas-panel">
              <svg className="graph-edge-layer" viewBox="0 0 100 100" preserveAspectRatio="none">
                {graph.edges.map((edge) => {
                  ...
                  const geometry = getGraphStageEdgeGeometry(from, to, graph.topology === 'directed');
                  ...
                  return (
                    <g key={edgeKey}>
                      ...
                      {graph.topology === 'directed' ? (
                        <path
                          className={`graph-edge-outline-arrow${
                            isCompleted ? ' graph-edge-outline-arrow-completed' : ''
                          }${isActive ? ' graph-edge-outline-arrow-active' : ''}`}
              ...
              <div className="tree-node-layer graph-node-layer">
                {graph.nodes.map((node, index) => {
                  ...
                  const isVisited = visitOrderIndexMap.has(index);
                  const isOnStack = stackSet.has(index);
                    ...
                    >
                      ...
                      {isVisited ? (
                        <span className="tree-node-badge graph-node-badge-visited">
                          {visitOrderIndexMap.get(index)}
                        </span>
                      ) : null}
                      <span className="tree-node-value">{node.id}</span>
                      <span className="tree-node-index">
                        {isCompleted
                          ? t('module.g02.node.backtracked')
                          : isActive
                            ? t('module.g02.node.current')
                            : isOnStack
                              ? t('module.g02.node.stack')
                              : isVisited
                                ? t('module.g02.node.visited')
                                : t('module.g02.node.idle')}
                      </span>
                    </div>
                  );
                })}
            ...
            <div className="graph-stage-views">
              <section className="graph-stage-view-card">
                <div className="graph-stage-view-head">
                  <strong>{t('module.g02.view.adjacency')}</strong>
                </div>
                ...
                <div className="graph-list-rows">
                  {adjacencyList.map((neighbors, rowIndex) => {
                    ...
                    const isVisitedRow = visitOrderIndexMap.has(rowIndex);
                        ...
                        <div className="graph-list-row-values">
                          {neighbors.length === 0 ? (
                            ...
                            neighbors.map((neighborIndex) => {
                              ...
                              const neighborVisited = visitOrderIndexMap.has(neighborIndex);
              ...
              <section className="graph-stage-view-card">
                <div className="graph-stage-view-head">
                  <strong>{t('module.g02.view.state')}</strong>
                </div>

                <div className="graph-dfs-state-grid">
                  <section className="graph-dfs-state-block">
                    <span className="graph-dfs-block-title">{t('module.g02.view.visitOrder')}</span>
                    {graph && currentSnapshot && currentSnapshot.outputOrder.length > 0 ? (
                      ...
                      <span className="tree-workspace-sequence-empty">{t('module.g02.state.empty.output')}</span>
                    )}
                  </section>

                  <section className="graph-dfs-state-block">
                    <span className="graph-dfs-block-title">{t('module.g02.view.callStack')}</span>
                    {graph && currentSnapshot && currentSnapshot.stackNodeIndices.length > 0 ? (
                      <div className="graph-dfs-stack-list">
                        {currentSnapshot.stackNodeIndices.map((nodeIndex, index) => (
                          <div
                            key={`${nodeIndex}-${index}`}
                            className={`graph-dfs-stack-item${
                              index === currentSnapshot.stackNodeIndices.length - 1
                                ? ' graph-dfs-stack-item-active'
                                : ''
                            }`}
                          >
                            <span className="graph-dfs-stack-depth">d{index}</span>
                            <strong>{graph.nodes[nodeIndex]?.id ?? '?'}</strong>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <span className="graph-dfs-stack-empty">{t('module.g02.state.empty.stack')}</span>
                    )}
                  </section>
                </div>
              </section>
      ...
      transportRight={
        <>
          ...
          <span className="tree-workspace-transport-chip">
            {t('module.g02.meta.completedCount')}: {completedCount}/{totalNodes}
          </span>
          <span className="tree-workspace-transport-chip">
            {t('module.g02.meta.stackDepth')}: {stackDepth}
          </span>
          <span className="tree-workspace-transport-chip tree-workspace-transport-chip-active">
            {t('module.g02.meta.activeRelation')}: {activeRelationLabel}
          </span>
        </>
      }
    />
