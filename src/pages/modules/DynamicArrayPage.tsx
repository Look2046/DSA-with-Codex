...
const DYNAMIC_ARRAY_C_LINES = [
  'if (size < 0 || size > capacity) return ERROR;',
  'if (size < capacity) { data[size] = value; size++; return OK; }',
  '    newCapacity = capacity * 2;',
  '    newData = malloc(newCapacity * sizeof(int));',
  '    for (int i = 0; i < size; ++i) newData[i] = data[i];',
  '    free(data); data = newData; capacity = newCapacity;',
  '    data[size] = value; size++; return OK;',
] as const;
      ...
      controlsContent={
        <>
          <div className="array-controls-grid">
            ...
            <label className="tree-workspace-field array-controls-field array-controls-field-speed" htmlFor="speed-select-DynamicArrayPage">
              <span>{t('module.s01.speed')}</span>
              <select
                id="speed-select-DynamicArrayPage"
                value={speedMs}
