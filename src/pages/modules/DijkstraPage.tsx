...
function getPresetLabel(presetId: WeightedGraphPresetId, t: TranslateFn): string {
  switch (presetId) {
    case 'positiveDirected':
      return t('module.g04.preset.positiveDirected');
    case 'positiveUndirected':
      return t('module.g04.preset.positiveUndirected');
    default:
      return presetId;
  }
}
...
function getRelationLabel(graph: WeightedGraphDefinition, fromIndex: number | null, toIndex: number | null): string {
  ...
  return graph.topology === 'undirected' ? `${left} - ${right}` : `${left} -> ${right}`;
}

function formatEdgeList(graph: WeightedGraphDefinition): string {
  return graph.edges
    .map((edge) => {
      const connector = graph.topology === 'undirected' ? '-' : '->';
      return `${graph.nodes[edge.from]?.id ?? '?'} ${connector} ${graph.nodes[edge.to]?.id ?? '?'} (${edge.weight})`;
    })
    .join(', ');
}
    ...
    <WorkspaceShell
      ...
      controlsContent={
        <>
          ...
          <label className="tree-workspace-field" htmlFor="speed-select-DijkstraPage">
            <span>{t('module.s01.speed')}</span>
            <select
              id="speed-select-DijkstraPage"
              value={speedMs}
