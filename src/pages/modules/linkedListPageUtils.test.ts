...
describe('linkedListPageUtils', () => {
  ...
  it('validates insert and delete index ranges', () => {
    const insertError = resolveLinkedListConfig('singly', '1,2', 'insertAt', '9', '0', t);
    const deleteError = resolveLinkedListConfig('singly', '1,2', 'deleteAt', '9', '3', t);
  ...
  it('treats delete on an empty list with index 1 as a valid no-op config', () => {
    const resolved = resolveLinkedListConfig('singly', '', 'deleteAt', '', '1', t);
  ...
  it('allows deleting the only node of a single-element list and rejects out-of-range index', () => {
    const singleValid = resolveLinkedListConfig('singly', '5', 'deleteAt', '', '1', t);
    const singleOverflow = resolveLinkedListConfig('singly', '5', 'deleteAt', '', '2', t);
    const multi = resolveLinkedListConfig('singly', '5,6', 'deleteAt', '', '2', t);
  ...
  it('serializes and resolves linked-list JSON config', () => {
    ...
    expect(resolveLinkedListConfigFromJson(serialized, t)).toEqual({
      config: { mode: 'doubly', list: [4, 7, 11], operation: { type: 'insertAt', index: 1, value: 9 } },
      error: '',
    });
  });

  it('resolves legacy linked-list JSON as singly mode', () => {
    const resolved = resolveLinkedListConfigFromJson(
      '{"list":[4,7,11],"operation":{"type":"find","value":7}}',
      t,
    );

    expect(resolved.config?.mode).toBe('singly');
  });
