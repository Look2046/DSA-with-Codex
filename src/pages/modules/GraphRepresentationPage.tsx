...
import { buildGraphRepresentationTimelineFromPreset } from '../../modules/graph/graphRepresentationTimelineAdapter';
import {
  getGraphEdgeKey,
  getGraphPresetIds,
  type GraphDefinition,
  type GraphPresetId,
  type GraphRepresentationStep,
...
const CODE_LINE_KEYS = [
  'module.g01.code.line1',
  'module.g01.code.line2',
  'module.g01.code.line3',
  'module.g01.code.line4',
  'module.g01.code.line5',
  'module.g01.code.line6',
  'module.g01.code.line7',
] as const;
const SPEED_OPTIONS = [
...
function getStepDescription(step: GraphRepresentationStep | undefined, t: TranslateFn): string {
  if (!step) {
    return '-';
  }
  ...
  switch (step.action) {
    case 'initial':
      return t('module.g01.step.initial');
    case 'selectVertex':
      return t('module.g01.step.selectVertex');
    case 'inspectEdge':
      return t('module.g01.step.inspectEdge');
    case 'completeRow':
      return t('module.g01.step.completeRow');
    case 'completed':
      return t('module.g01.step.completed');
    default:
      return '-';
  }
}
...
export function GraphRepresentationPage() {
  ...
  const timelineFrames = useMemo(() => buildGraphRepresentationTimelineFromPreset(presetId), [presetId]);
  const steps = useMemo(() => timelineFrames.map((frame) => frame.payload), [timelineFrames]);
  const currentStep = currentFrame;
  const currentSnapshot = steps[currentStep] ?? steps[0];
  const graph = currentSnapshot?.graph ?? null;
  const adjacencyList = currentSnapshot?.adjacencyList ?? [];
  const adjacencyMatrix = currentSnapshot?.adjacencyMatrix ?? [];
  ...
  const completedEdgeSet = useMemo(
    () => new Set(currentSnapshot?.completedEdgeKeys ?? []),
    [currentSnapshot?.completedEdgeKeys],
  );
  ...
  const currentRowText =
    graph && currentSnapshot?.activeNodeIndex !== null && currentSnapshot?.activeNodeIndex !== undefined
      ...
      : '-';
  const matrixCellText =
    graph && currentSnapshot?.activeMatrixCell
      ? `${graph.nodes[currentSnapshot.activeMatrixCell.row]?.id ?? '?'} , ${
          graph.nodes[currentSnapshot.activeMatrixCell.col]?.id ?? '?'
        }`
      : '-';
  const mirroredCellText =
    graph && currentSnapshot?.mirroredMatrixCell
      ? `${graph.nodes[currentSnapshot.mirroredMatrixCell.row]?.id ?? '?'} , ${
          graph.nodes[currentSnapshot.mirroredMatrixCell.col]?.id ?? '?'
        }`
      : '-';
  const completedRows = currentSnapshot?.completedNodeIndices.length ?? 0;
  const totalRows = graph?.nodes.length ?? 0;
  const inspectedRelationCount = currentSnapshot?.inspectedRelationCount ?? 0;
  const totalRelationCount = currentSnapshot?.totalRelationCount ?? 0;
  const isAtLastFrame = steps.length === 0 || currentStep >= steps.length - 1;

  const detailParts = [
    `${t('module.g01.meta.topology')}: ${graph ? getTopologyLabel(graph, t) : '-'}`,
    `${t('module.g01.meta.rowsCompleted')}: ${completedRows}/${totalRows}`,
    `${t('module.g01.meta.relationsMapped')}: ${inspectedRelationCount}/${totalRelationCount}`,
  ];

  if (activeRelationLabel !== '-') {
    detailParts.push(`${t('module.g01.meta.activeRelation')}: ${activeRelationLabel}`);
  }
  if (matrixCellText !== '-') {
    detailParts.push(`${t('module.g01.meta.matrixCell')}: ${matrixCellText}`);
  }
  if (mirroredCellText !== '-') {
    detailParts.push(`${t('module.g01.meta.mirroredCell')}: ${mirroredCellText}`);
  }
  ...
  return (
    <WorkspaceShell
      ...
      description={t('module.g01.body')}
      stageAriaLabel={t('module.g01.stage')}
      ...
      stageMeta={
        <>
          ...
          <span className="tree-workspace-pill">
            {t('module.g01.meta.preset')}: {graph ? getPresetLabel(graph.presetId, t) : '-'}
          </span>
          <span className="tree-workspace-pill">
            {t('module.g01.meta.topology')}: {graph ? getTopologyLabel(graph, t) : '-'}
          </span>
          <span className="tree-workspace-pill">
            {t('module.g01.meta.activeNode')}: {activeNodeLabel}
          </span>
          ...
          {graph ? (
            <>
              ...
              <div className="tree-workspace-sample-block">
                ...
                <code>
                  {t('module.g01.meta.vertexCount')}: {graph.nodes.length} · {t('module.g01.meta.edgeCount')}:{' '}
                  {graph.edges.length}
                </code>
              </div>
            </>
      ...
      stepContent={
        <>
          ...
          <dl className="tree-workspace-kv">
            ...
            <div>
              <dt>{t('module.g01.meta.preset')}</dt>
              <dd>{graph ? getPresetLabel(graph.presetId, t) : '-'}</dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.topology')}</dt>
              <dd>{graph ? getTopologyLabel(graph, t) : '-'}</dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.activeNode')}</dt>
              <dd>{activeNodeLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.activeNeighbor')}</dt>
              <dd>{activeNeighborLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.activeRelation')}</dt>
              <dd>{activeRelationLabel}</dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.currentRow')}</dt>
              <dd>{currentRowText}</dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.matrixCell')}</dt>
              <dd>{matrixCellText}</dd>
            </div>
            {mirroredCellText !== '-' ? (
              <div>
                <dt>{t('module.g01.meta.mirroredCell')}</dt>
                <dd>{mirroredCellText}</dd>
              </div>
            ) : null}
            <div>
              <dt>{t('module.g01.meta.rowsCompleted')}</dt>
              <dd>
                {completedRows}/{totalRows}
              </dd>
            </div>
            <div>
              <dt>{t('module.g01.meta.relationsMapped')}</dt>
              <dd>
                {inspectedRelationCount}/{totalRelationCount}
              </dd>
            </div>
          </dl>

          <div className="tree-workspace-code-block">
            <span className="tree-workspace-code-title">{t('module.g01.code.title')}</span>
      ...
      stageContent={
        graph ? (
          <div className="graph-stage-scene" aria-hidden="true">
            <section className="graph-stage-canvas-panel">
              <span className="graph-stage-label graph-stage-label-canvas">{t('module.g01.view.canvas')}</span>
              ...
              <svg className="graph-edge-layer" viewBox="0 0 100 100" preserveAspectRatio="none">
                ...
                {graph.edges.map((edge) => {
                  ...
                  const isCompleted = completedEdgeSet.has(edgeKey);
              ...
              <div className="tree-node-layer graph-node-layer">
                {graph.nodes.map((node, index) => {
                  ...
                  return (
                    <div
                      key={node.id}
                      className={`tree-node graph-node${isCompleted ? ' graph-node-completed' : ''}${
                    ...
                    >
                      ...
                      <span className="tree-node-index">
                        {isCompleted
                          ? t('module.g01.node.completed')
                          : isActive
                            ? t('module.g01.node.active')
                            : isNeighbor
                              ? t('module.g01.node.neighbor')
                              : t('module.g01.node.idle')}
                      </span>
                    </div>
                  );
                })}
            ...
            <div className="graph-stage-views">
              <section className="graph-stage-view-card">
                <div className="graph-stage-view-head">
                  <strong>{t('module.g01.view.list')}</strong>
                  <span>{t('module.g01.view.listHint')}</span>
                </div>
                    ...
                    return (
                      <div
                        ...
                        className={`graph-list-row${isActiveRow ? ' graph-list-row-active' : ''}${
                          isCompletedRow ? ' graph-list-row-completed' : ''
                        }`}
                      >
                              ...
                              return (
                                <span
                                  ...
                                  className={`graph-list-chip${isActiveNeighbor ? ' graph-list-chip-active' : ''}${
                                    isCompletedRow ? ' graph-list-chip-completed' : ''
                                  }`}
                                >
              ...
              <section className="graph-stage-view-card">
                <div className="graph-stage-view-head">
                  <strong>{t('module.g01.view.matrix')}</strong>
                  <span>{t('module.g01.view.matrixHint')}</span>
                </div>
                    ...
                    <thead>
                      <tr>
                        ...
                        {graph.nodes.map((node) => (
                          <th key={`head-${node.id}`}>{node.id}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <tr
                            ...
                            className={`${isActiveRow ? ' graph-matrix-row-active' : ''}${
                              isCompletedRow ? ' graph-matrix-row-completed' : ''
                            }`}
                          >
                            <th>{graph.nodes[rowIndex]?.id ?? '?'}</th>
                            {row.map((value, colIndex) => {
                              const isActiveCell =
                                currentSnapshot?.activeMatrixCell?.row === rowIndex &&
                                currentSnapshot.activeMatrixCell.col === colIndex;
                              const isMirroredCell =
                                currentSnapshot?.mirroredMatrixCell?.row === rowIndex &&
                                currentSnapshot.mirroredMatrixCell.col === colIndex;

                              return (
                                <td
                                  key={`${rowIndex}-${colIndex}`}
                                  className={`${isActiveCell ? ' graph-matrix-cell-active' : ''}${
                                    isMirroredCell ? ' graph-matrix-cell-mirror' : ''
                                  }`}
                                >
                                  {value}
                                </td>
      ...
      transportRight={
        <>
          ...
          <span className="tree-workspace-transport-chip tree-workspace-transport-chip-active">
            {t('module.g01.meta.activeRelation')}: {activeRelationLabel}
          </span>
        </>
      }
    />
