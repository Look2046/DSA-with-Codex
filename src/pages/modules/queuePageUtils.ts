...
export function getStepDescription(step: QueueStep | undefined, t: Translator): string {
  ...
  if (step.action === 'prepareEnqueue') {
    return `${t('module.l05.step.prepareEnqueue')} ${step.enqueuedValue ?? ''}`.trim();
  }
  if (step.action === 'linkEnqueue') {
    return `${t('module.l05.step.linkEnqueue')} ${step.enqueuedValue ?? ''}`.trim();
  }
