...
const EMPTY_QUEUE_STATE: number[] = [];
const EMPTY_QUEUE_BUFFER: Array<number | null> = [];

const QUEUE_PSEUDOCODE = {
  normal: {
    enqueue: [
      { sourceLine: 1, textKey: 'module.l05.code.normal.enqueue.line1', cLine: 'if (rear > MAXSIZE || front > rear) return ERROR;' },
      { sourceLine: 1, textKey: 'module.l05.code.normal.enqueue.line2', cLine: 'if (rear == MAXSIZE) return OVERFLOW;' },
      ...
      { sourceLine: 6, textKey: 'module.l05.code.normal.enqueue.line5', cLine: 'return OK;' },
    ],
    dequeue: [
      { sourceLine: 1, textKey: 'module.l05.code.normal.dequeue.line1', cLine: 'if (front == rear) return UNDERFLOW;' },
      { sourceLine: 4, textKey: 'module.l05.code.normal.dequeue.line2', cLine: 'x = data[front]; data[front] = EMPTY;' },
      { sourceLine: 4, textKey: 'module.l05.code.normal.dequeue.line3', cLine: 'front = front + 1;' },
      { sourceLine: 6, textKey: 'module.l05.code.normal.dequeue.line4', cLine: 'return x;' },
    ],
    front: [
      { sourceLine: 1, textKey: 'module.l05.code.normal.front.line1', cLine: 'if (front == rear) return EMPTY;' },
      { sourceLine: 5, textKey: 'module.l05.code.normal.front.line2', cLine: 'return data[front];' },
      { sourceLine: 6, textKey: 'module.l05.code.normal.front.line3', cLine: '/* front value was returned without moving front */' },
    ],
  },
  linked: {
    enqueue: [
      { sourceLine: 1, textKey: 'module.l05.code.linked.enqueue.line1', cLine: 'QNode *s = createNode(value); s->next = NULL;' },
      { sourceLine: 2, textKey: 'module.l05.code.linked.enqueue.line2', cLine: 'if (rear == NULL) front = rear = s;' },
      { sourceLine: 3, textKey: 'module.l05.code.linked.enqueue.line3', cLine: 'else { rear->next = s; rear = s; }' },
      { sourceLine: 4, textKey: 'module.l05.code.linked.enqueue.line4', cLine: 'return OK;' },
    ],
    dequeue: [
      { sourceLine: 1, textKey: 'module.l05.code.linked.dequeue.line1', cLine: 'if (front == NULL) return UNDERFLOW;' },
      { sourceLine: 4, textKey: 'module.l05.code.linked.dequeue.line2', cLine: 'QNode *p = front; x = p->data;' },
      { sourceLine: 4, textKey: 'module.l05.code.linked.dequeue.line3', cLine: 'front = front->next;' },
      { sourceLine: 4, textKey: 'module.l05.code.linked.dequeue.line4', cLine: 'if (front == NULL) rear = NULL;' },
      { sourceLine: 6, textKey: 'module.l05.code.linked.dequeue.line5', cLine: 'free(p); return x;' },
    ],
    front: [
      { sourceLine: 1, textKey: 'module.l05.code.linked.front.line1', cLine: 'if (front == NULL) return EMPTY;' },
      { sourceLine: 5, textKey: 'module.l05.code.linked.front.line2', cLine: 'return front->data;' },
    ],
  },
  circular: {
    enqueue: [
      { sourceLine: 1, textKey: 'module.l05.code.circular.enqueue.line1', cLine: 'if ((rear + 1) % MAXSIZE == front) return OVERFLOW;' },
      ...
      { sourceLine: 6, textKey: 'module.l05.code.circular.enqueue.line4', cLine: 'return OK;' },
    ],
    dequeue: [
      { sourceLine: 1, textKey: 'module.l05.code.circular.dequeue.line1', cLine: 'if (front == rear) return UNDERFLOW;' },
      { sourceLine: 4, textKey: 'module.l05.code.circular.dequeue.line2', cLine: 'x = data[front]; data[front] = EMPTY;' },
      { sourceLine: 4, textKey: 'module.l05.code.circular.dequeue.line3', cLine: 'front = (front + 1) % MAXSIZE;' },
      { sourceLine: 6, textKey: 'module.l05.code.circular.dequeue.line4', cLine: 'return x;' },
    ],
    front: [
      { sourceLine: 1, textKey: 'module.l05.code.circular.front.line1', cLine: 'if (front == rear) return EMPTY;' },
      { sourceLine: 5, textKey: 'module.l05.code.circular.front.line2', cLine: 'return data[front];' },
      { sourceLine: 6, textKey: 'module.l05.code.circular.front.line3', cLine: '/* front value was returned without moving front */' },
    ],
  },
} as const;

type QueueMode = 'normal' | 'linked' | 'circular';
type QueuePageMode = QueueMode;
const QUEUE_WORKSPACE_CONFIG = getQueueWorkspaceConfig();

const QUEUE_TITLE_KEYS: Record<QueuePageMode, TranslationKey> = {
  normal: 'module.l05a.title',
  linked: 'module.l05b.title',
  circular: 'module.l05c.title',
};

const QUEUE_BODY_KEYS: Record<QueuePageMode, TranslationKey> = {
  normal: 'module.l05a.body',
  linked: 'module.l05b.body',
  circular: 'module.l05c.body',
};
...
export function QueuePage() {
  return <QueueWorkbenchPage fixedMode="normal" />;
}

export function SequentialQueuePage() {
  return <QueueWorkbenchPage fixedMode="normal" />;
}

export function LinkedQueuePage() {
  return <QueueWorkbenchPage fixedMode="linked" />;
}

export function CircularQueuePage() {
  return <QueueWorkbenchPage fixedMode="circular" />;
}
...
function QueueWorkbenchPage({ fixedMode }: { fixedMode: QueuePageMode }) {
  const { t } = useI18n();
  ...
  const runtimeMode: QueueRuntimeMode = mode;
  ...
  const activeTitle = t(QUEUE_TITLE_KEYS[mode]);
  const activeDescription = t(QUEUE_BODY_KEYS[mode]);
  const currentModeLabel =
    mode === 'linked' ? t('module.l05.tab.linked') : mode === 'circular' ? t('module.l05.tab.circular') : t('module.l05.tab.sequential');
  ...
  const pseudocodeMode: QueueMode = mode;
  const activePseudocode = QUEUE_PSEUDOCODE[pseudocodeMode][operationType];
  const activeDisplayLines = activePseudocode
    .map((item, index) => (currentSnapshot?.codeLines.includes(item.sourceLine) ? index + 1 : -1))
    .filter((line) => line > 0);
  const queueState = currentSnapshot?.queueState ?? EMPTY_QUEUE_STATE;
  const bufferState = currentSnapshot?.bufferState ?? EMPTY_QUEUE_BUFFER;
  ...
  const handleResetOperation = useCallback(() => {
    ...
    const nextRuntimeMode: QueueRuntimeMode = mode;
  ...
  const linkedIncomingValue = currentSnapshot?.enqueuedValue;
  const showLinkedIncomingNode =
    mode === 'linked' &&
    operationType === 'enqueue' &&
    (currentSnapshot?.action === 'prepareEnqueue' || currentSnapshot?.action === 'linkEnqueue');
  const showLinkedIncomingConnector = currentSnapshot?.action === 'linkEnqueue' && queueState.length > 0;
  const linkedIncomingIndex = queueState.length;

  const stageVisualization = mode === 'linked' ? (
    <div className="linked-queue-scene" aria-label="linked-queue-cells" ref={queueCellsRef}>
      <div className="linked-queue-main-row">
        {queueState.length > 0 ? (
          queueState.map((value, index) => {
            const highlight = highlightMap.get(index) ?? highlightMap.get(frontIndex + index) ?? 'default';
            const isFront = index === 0;
            const isRear = index === queueState.length - 1;

            return (
              <div key={`linked-queue-${index}-${value}`} className="linked-queue-item">
                <div data-queue-index={index} className={`linked-queue-node bar-${highlight}`}>
                  {isFront ? <span className="linked-queue-pointer linked-queue-pointer-front">{t('module.l05.front')}</span> : null}
                  {isRear ? <span className="linked-queue-pointer linked-queue-pointer-rear">{t('module.l05.rear')}</span> : null}
                  <span className="array-cell-index">{index}</span>
                  <strong>{value}</strong>
                </div>
                {!isRear ? <span className="linked-queue-connector" aria-hidden="true" /> : null}
              </div>
            );
          })
        ) : !showLinkedIncomingNode ? (
          <div className="queue-empty">{t('module.l05.empty')}</div>
        ) : null}
      </div>
      {showLinkedIncomingNode && linkedIncomingValue !== undefined ? (
        <div className="linked-queue-item linked-queue-item-incoming">
          {showLinkedIncomingConnector ? <span className="linked-queue-connector linked-queue-connector-incoming" aria-hidden="true" /> : null}
          <div className="linked-queue-node linked-queue-node-floating bar-new-node">
            <span className="linked-queue-variable-badge">s</span>
            {queueState.length === 0 && currentSnapshot?.action === 'linkEnqueue' ? (
              <>
                <span className="linked-queue-pointer linked-queue-pointer-front">{t('module.l05.front')}</span>
                <span className="linked-queue-pointer linked-queue-pointer-rear">{t('module.l05.rear')}</span>
              </>
            ) : null}
            <span className="array-cell-index">{linkedIncomingIndex}</span>
            <strong>{linkedIncomingValue}</strong>
          </div>
        </div>
      ) : null}
      {queueState.length === 0 && showLinkedIncomingNode ? (
        <div className="queue-empty queue-empty-inline">{t('module.l05.empty')}</div>
      ) : null}
    </div>
  ) : mode === 'circular' ? (
      ...
      controlsContent={
        <>
          <div className="array-controls-grid queue-controls-grid">
            ...
            <label className="tree-workspace-field array-controls-field array-controls-field-speed" htmlFor="speed-select-QueuePage">
                <span>{t('module.s01.speed')}</span>
                <select
                  id="speed-select-QueuePage"
                  value={speedMs}
