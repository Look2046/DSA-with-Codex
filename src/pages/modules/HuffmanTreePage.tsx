...
const BUILD_C_LINES = [
  'void HaffmanTree(HNodeType HuffNode[MAXLEAF]) {',
  '    int i, j, m1, m2, x1, x2;',
  '    for (i = 0; i < 2 * n - 1; ++i) {',
  '        HuffNode[i].weight = 0; HuffNode[i].parent = -1;',
  '        HuffNode[i].lchild = -1; HuffNode[i].rchild = -1;',
  '    }',
  '    for (i = 0; i < n; ++i) read HuffNode[i].weight;',
  '    for (i = 0; i < n - 1; ++i) {',
  '        m1 = m2 = MAXVALUE; x1 = x2 = 0;',
  '        for (j = 0; j < n + i; ++j) { /* select two minima with parent == -1 */ }',
...
const CODE_DETAIL_C_LINES = [
  'wait until huffmanTree is complete;',
  'for each leaf in tree {',
  '    walk from root to this leaf;',
  '    append 0 on left and 1 on right;',
  '    output leaf.code;',
  '}',
] as const;
