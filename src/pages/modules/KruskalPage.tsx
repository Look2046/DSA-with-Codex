...
export function KruskalPage() {
  ...
  const processedSortedEdges = useMemo(() => {
    if (!currentSnapshot) {
      return [];
    }

    if (currentSnapshot.activeEdgeIndex !== null) {
      return currentSnapshot.sortedEdges.slice(0, currentSnapshot.activeEdgeIndex + 1);
    }

    if (currentSnapshot.action === 'completed') {
      return currentSnapshot.sortedEdges.filter((edge) => {
        const edgeKey = getWeightedGraphEdgeKey(currentSnapshot.graph, edge.from, edge.to);
        return selectedEdgeSet.has(edgeKey) || rejectedEdgeSet.has(edgeKey);
      });
    }

    return [];
  }, [currentSnapshot, rejectedEdgeSet, selectedEdgeSet]);
    ...
    <WorkspaceShell
      ...
      controlsContent={
        <>
          ...
          <label className="tree-workspace-field" htmlFor="speed-select-KruskalPage">
            <span>{t('module.s01.speed')}</span>
            <select
              id="speed-select-KruskalPage"
              value={speedMs}
      ...
      stageContent={
        graph ? (
          <div className="graph-bfs-stage-scene graph-kruskal-stage-scene" aria-hidden="true">
            <div className="graph-kruskal-left-stack">
              <section className="graph-stage-canvas-panel">
                <div className="graph-kruskal-graph-pan-layer" style={graphPan.panStyle} {...graphPan.panHandlers}>
                  <svg className="graph-edge-layer" viewBox="0 0 100 100" preserveAspectRatio="none">
                    {graph.edges.map((edge) => {
                      ...
                      const isCandidate = isActive && !isSelected && !isRejected;
                      const isPending = !isSelected && !isRejected && !isCandidate;
                      ...
                      return (
                        <g key={edgeKey}>
                          ...
                          }${isCandidate ? ' graph-edge-candidate' : ''}${isPending ? ' graph-edge-pending' : ''}`}
                            x1={from?.x ?? 0}
                            y1={from?.y ?? 0}
                            x2={to?.x ?? 0}
                            y2={to?.y ?? 0}
                          ...
                          }${isCandidate ? ' graph-edge-weight-candidate' : ''}${isPending ? ' graph-edge-weight-pending' : ''}`}
                          >
                            {edge.weight}
                          </text>
                        </g>
            ...
            <aside className="graph-kruskal-right-stack">
              <section className="graph-stage-view-card graph-kruskal-components-panel">
                <div className="graph-stage-view-head">
                  <strong>{t('module.g07.view.components')}</strong>
                </div>

                <div className="graph-distance-rows graph-kruskal-components-list">
                  <div className="graph-distance-row graph-distance-row-active graph-kruskal-summary-row">
                  ...
                  {currentSnapshot.componentGroups.map((group, index) => (
                    <div key={`group-${index}`} className="graph-distance-row graph-kruskal-component-row">
                      <strong>{`${t('module.g07.component.label')} ${index + 1}`}</strong>
                      <div className="graph-floyd-chip-row graph-kruskal-component-chip-row">
                        {group.map((nodeIndex) => (
                          <span key={`${index}-${nodeIndex}`} className="graph-floyd-chip graph-kruskal-component-chip">
              ...
              <section className="graph-stage-view-card graph-kruskal-edge-order-panel">
                <div className="graph-stage-view-head">
                  <strong>{t('module.g07.view.edgeOrder')}</strong>
                </div>

                <div className="graph-distance-rows graph-kruskal-edge-order-list">
                  {processedSortedEdges.length === 0 ? (
                    <p className="graph-kruskal-edge-empty">{t('module.g07.view.noProcessedEdges')}</p>
                  ) : null}

                  {processedSortedEdges.map((edge) => {
                    const edgeKey = getWeightedGraphEdgeKey(graph, edge.from, edge.to);
                    const index = currentSnapshot.sortedEdges.findIndex(
                      (candidate) => getWeightedGraphEdgeKey(graph, candidate.from, candidate.to) === edgeKey,
                    );
                    ...
                    return (
                      <div
                        key={`${edgeKey}-${index}`}
                        className={`graph-distance-row graph-kruskal-edge-row${
                          isCandidate ? ' graph-distance-row-active graph-kruskal-edge-row-candidate' : ''
                        }${isSelected ? ' graph-distance-row-relaxed graph-kruskal-edge-row-selected' : ''}${
                          isRejected ? ' graph-distance-row-rejected graph-kruskal-edge-row-rejected' : ''
                        }`}
                      >
